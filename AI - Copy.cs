...
public class AI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        _agentSenses.GetEnemiesInView(); //lists enemies in view


        if (_agentData.HasEnemyFlag ) //goes back to base when enemy flag is captured
        {
            _agentActions.MoveTo(_agentData.FriendlyBase);
        }

        if (Input.GetKeyDown(KeyCode.F)) //temp keybind for health kit usage
        {
            _agentActions.UseItem(_agentInventory.GetItem("Health Kit"));
            _agentInventory.RemoveItem("Health Kit");
        }

        if (Input.GetKeyDown(KeyCode.G)) //temp keybind for power up usage
        {
            _agentActions.UseItem(_agentInventory.GetItem("Power Up"));
            _agentInventory.RemoveItem("Power Up");
        }

        if (TargetsObj == null) // starts target function to attack nearest enemy
        {
            FindTarget();
        }
        else
        {
            _agentActions.AttackEnemy(TargetsObj);
        }

        if (_agentSenses.IsItemInReach(_agentData.EnemyBase))
        {
    ...
    void FindTarget()
    {
        ...
        {
            TargetsObj = LeaderObj.GetComponent<AI>().TargetsObj;
        }
